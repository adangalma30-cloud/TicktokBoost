...
class StabilityTest {
    ...
    @Test
    fun `premium badge stays horizontal and readable beside long names`() {
        goHome()
        rule.onNodeWithText("Discover").performClick()
        rule.waitForIdle()
        rule.mainClock.advanceTimeBy(900)
        rule.waitForIdle()
        // Trusted filter ranks the PRO creator (trust 94) first -> badge is composed
        rule.onNodeWithText("Trusted").performClick()
        rule.waitForIdle()
        rule.mainClock.advanceTimeBy(400)
        rule.waitForIdle()
        // badge label renders as ONE intact single-line node — never one letter per line
        rule.onNodeWithText("Pro").assertExists()
        // the extreme-length fixture is present in discovery
        org.junit.Assert.assertTrue(
            com.tiktokboost.app.data.MockData.users.any { it.id == "u19" && it.displayName.length > 40 }
        )
        // short-name fixture also present (layout must handle both)
        org.junit.Assert.assertTrue(
            com.tiktokboost.app.data.MockData.users.any { it.id == "u20" && it.displayName.length <= 3 }
        )
    }

    @Test
    fun `profile completion shows clean action text`() {
        goHome()
        rule.onNodeWithText("Profile").performClick()
        rule.waitForIdle()
        rule.mainClock.advanceTimeBy(400)
